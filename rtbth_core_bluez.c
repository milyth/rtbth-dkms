...
int rtbt_hci_dev_ioctl(struct hci_dev *hdev, unsigned int cmd,
		       unsigned long arg)
{
	BT_WARN("%s(dev=0x%lx): ioctl cmd=0x%x", __FUNCTION__, (ULONG)hdev,
		cmd);
	return -ENOIOCTLCMD;
}
#endif
...
int rtbt_hci_dev_send(struct sk_buff *skb)
{
	...
	struct rtbt_os_ctrl *os_ctrl =
		(struct rtbt_os_ctrl *)hci_get_drvdata(hdev);
	...
	case HCI_COMMAND_PKT:
		status = hps_ops->hci_cmd(os_ctrl->dev_ctrl, skb->data,
					  skb->len);
		if (hdev && (status == 0))
			hdev->stat.cmd_tx++;
		break;
	case HCI_ACLDATA_PKT:
		status = hps_ops->hci_acl_data(os_ctrl->dev_ctrl, skb->data,
					       skb->len);
		if (hdev && (status == 0))
			hdev->stat.acl_tx++;
		break;
	case HCI_SCODATA_PKT:
		BT_DBG("%s(): sco len=%d,time=0x%lx", __FUNCTION__, skb->len,
		       jiffies);
	...
	os_ctrl->sco_tx_seq = bt_cb(skb)->tx_seq;
		...
		status = hps_ops->hci_sco_data(os_ctrl->dev_ctrl, skb->data,
					       skb->len);
		if (hdev && (status == 0))
			hdev->stat.sco_tx++;
...
int rtbt_hci_dev_receive(void *bt_dev, int pkt_type, char *buf, int len)
{
	...
	if (pkt_type == HCI_SCODATA_PKT)
		BT_DBG("%s(): send sco data to OS, time=0x%lx", __FUNCTION__,
		       jiffies);
...
int rtbt_hci_dev_close(struct hci_dev *hdev)
{
	...
	if (!test_and_clear_bit(HCI_RUNNING, &(hdev->flags))) {
		BT_WARN("%s(): Directly return due to hdev->flags=0x%lx",
...
int rtbt_hps_iface_detach(IN struct rtbt_os_ctrl *os_ctrl)
{
	...
	if (!hdev) {
		BT_ERR("%s(): os_ctrl(%p)->bt_dev is NULL", __FUNCTION__,
		       os_ctrl);
		return -1;
	}
...
int rtbt_hps_iface_deinit(IN int if_type, IN void *if_dev,
...
int rtbt_hps_iface_init(IN int if_type, IN void *if_dev,
			IN struct rtbt_os_ctrl *os_ctrl)
{
	struct hci_dev *hdev;
