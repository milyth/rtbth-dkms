...
UINT32 RT3298_REG_ADDR[] = {
    0x0,   0x4,   0x8,   0xc,   0x10,  0x14,  0x18,  0x1c,  0x20,  0x180, 0x184,
    0x188, 0x18c, 0x200, 0x204, 0x208, 0x20c, 0x210, 0x220, 0x228, 0x300, 0x304,
    0x308, 0x30c, 0x320, 0x324, 0x328, 0x32c, 0x330, 0x334, 0x338, 0x340, 0x344,
    0x348, 0x350, 0x354, 0x358, 0x35c, 0x360, 0x380, 0x384, 0x3c0, 0x3c4, 0x400,
    0x404, 0x408, 0x40c, 0x410, 0x414, 0x418, 0x41c, 0x420, 0x424, 0x428, 0x42c,
    0x434, 0x440, 0x444, 0x448, 0x600, 0x604, 0x608, 0x60c, 0x610};
...
NTSTATUS eFuseWriteRegisters(IN PRTBTH_ADAPTER pAd, IN USHORT Offset,
  ...
  do {
    ...
    tmpOffset >>= 4;
    tmpOffset |= ((~((tmpOffset & 0x01) ^ (tmpOffset >> 1 & 0x01) ^
                     (tmpOffset >> 2 & 0x01) ^ (tmpOffset >> 3 & 0x01)))
                  << 6) &
                 0x40;
    tmpOffset |= ((~((tmpOffset >> 2 & 0x01) ^ (tmpOffset >> 3 & 0x01) ^
...
VOID RaiseClock(IN PRTBTH_ADAPTER pAd, IN ULONG *x) {
  printk("rtbth: Raising clock by: %ld", *x);
...
VOID LowerClock(IN PRTBTH_ADAPTER pAd, IN ULONG *x) {
  printk("rtbth: Lowering clock by: %ld", *x);
...
int rtbt_prom_read16(IN PRTBTH_ADAPTER pAd, IN USHORT Offset,
                     OUT USHORT *pData) {
  ULONG x;
...
VOID Bth_EEPROM_WRITE16(IN PRTBTH_ADAPTER pAd, IN USHORT Offset,
                        IN USHORT Data) {
  ULONG value = 0;
  LONG count = 0;
...
int rtbt_dev_ctrl_init(IN struct rtbt_os_ctrl **pDevCtrl, IN void *csr) {
  ...
  if (spin_status | event_status | timer_status) {
    DebugPrint(ERROR, DBG_INIT,
               ...
               spin_status, event_status, timer_status);
...
static VOID BthShutdown(IN PRTBTH_ADAPTER pAd) {
...
inline int rtbth_rx_packet(void *pdata, RXBI_STRUC rxbi, void *buf,
                           unsigned int len) {
...
inline int rtbth_bz_hci_send(void *pdata, void *buf, unsigned long len) {
  // hci enqueue
  unsigned char delimiter[] = {0xcc, 0xcc};
...
inline int rtbth_bz_acl_send(void *pdata, void *buf, unsigned long len) {
  unsigned char delimiter[] = {0xcc, 0xcc};
  unsigned int sz_need = 2 + 4 + len;
  int ret = 0;
  unsigned long cpuflags;
...
inline int rtbth_bz_sco_send(void *pdata, void *buf, unsigned long len) {
  unsigned char delimiter[] = {0xcc, 0xcc};
  unsigned int sz_need = 2 + 4 + len;
  int ret = 0;
  unsigned long cpuflags;
