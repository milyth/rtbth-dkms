...
UINT32 RT3298_REG_ADDR[] = {
	0x0,   0x4,   0x8,   0xc,   0x10,  0x14,  0x18,	 0x1c,	0x20,  0x180,
	0x184, 0x188, 0x18c, 0x200, 0x204, 0x208, 0x20c, 0x210, 0x220, 0x228,
	0x300, 0x304, 0x308, 0x30c, 0x320, 0x324, 0x328, 0x32c, 0x330, 0x334,
	0x338, 0x340, 0x344, 0x348, 0x350, 0x354, 0x358, 0x35c, 0x360, 0x380,
	0x384, 0x3c0, 0x3c4, 0x400, 0x404, 0x408, 0x40c, 0x410, 0x414, 0x418,
	0x41c, 0x420, 0x424, 0x428, 0x42c, 0x434, 0x440, 0x444, 0x448, 0x600,
	0x604, 0x608, 0x60c, 0x610
};

VOID eFusePhysicalReadRegisters(IN PRTBTH_ADAPTER pAd, IN UINT16 Offset,
				IN UINT16 Length, OUT UINT16 *pData)
{
	...
	// Step0. Write 10-bit of address to EFSROM_AIN (0x324, bit25:bit16). The
	// address must be 16-byte alignment.
	eFuseCtrlStruc.field.EFSROM_AIN = Offset & 0xfff0;

	// Step1. Write EFSROM_MODE (0x324, bit7:bit6) to 1.
	eFuseCtrlStruc.field.EFSROM_MODE = 1;

	// Step2. Write EFSROM_KICK (0x324, bit30) to 1 to kick-off physical read
	// procedure.
...
UCHAR eFuseReadRegisters(IN RTBTH_ADAPTER *pAd, IN UINT16 Offset,
			 IN UINT16 Length, OUT UINT16 *pData)
{
	EFUSE_CTRL_STRUC eFuseCtrlStruc;
	int i;
	...
	// Step0. Write 10-bit of address to EFSROM_AIN (0x324, bit25:bit16). The
	// address must be 16-byte alignment.
	//        The address must be 16-byte alignment. (This is to say, the last 4
	//        bits must be 0)
	eFuseCtrlStruc.field.EFSROM_AIN = Offset & 0xfff0;
...
VOID eFuseReadPhysical(IN RTBTH_ADAPTER *pAd, IN UINT16 *lpInBuffer,
		       IN ULONG nInBufferSize, OUT UINT16 *lpOutBuffer,
...
NTSTATUS eFuseRead(IN RTBTH_ADAPTER *pAd, IN UINT16 Offset, OUT UINT8 *pData,
		   IN UINT16 Length)
{
	...
	for (i = 0; i < Length; i += 2) {
		EFSROM_AOUT =
			eFuseReadRegisters(pAd, Offset + i, 2, &pOutBuf[i / 2]);
	}
	return Status;
}

VOID eFusePhysicalWriteRegisters(IN RTBTH_ADAPTER *pAd, IN USHORT Offset,
				 IN USHORT Length, OUT USHORT *pData)
{
	...
	// Step0. Write 16-byte of data to EFUSE_DATA0-3 (0x590-0x59C), where
	// EFUSE_DATA0 is the LSB DW, EFUSE_DATA3 is the MSB DW.
	...
	// Step1. Write 10-bit of address to EFSROM_AIN (0x324, bit25:bit16). The
	// address must be 16-byte alignment.
	eFuseCtrlStruc.field.EFSROM_AIN = Offset & 0xfff0;
	...
	// Step3. Write EFSROM_KICK (0x324, bit30) to 1 to kick-off physical write
	// procedure.
	eFuseCtrlStruc.field.EFSROM_KICK = 1;
	...
	// Step4. Polling EFSROM_KICK(0x324, bit30) until it become 0 again. It’s
	// done.
	i = 0;
...
VOID eFuseWritePhysical(IN RTBTH_ADAPTER *pAd, IN USHORT *lpInBuffer,
			IN ULONG nInBufferSize, OUT UCHAR *lpOutBuffer,
...
NTSTATUS eFuseWriteRegisters(IN PRTBTH_ADAPTER pAd, IN USHORT Offset,
			     IN USHORT Length, IN USHORT *pData)
{
	...
	DebugPrint(TRACE, DBG_HW_ACCESS,
		   "eFuseWriteRegisters Offset=0x%X, Length=%d, pData=0x%X\n",
		   Offset, Length, *pData);
	...
	if (EFSROM_AOUT == 0x3f) {
		...
		for (i = EFUSE_USAGE_MAP_START; i <= EFUSE_USAGE_MAP_END;
		     i += 2) {
			eFusePhysicalReadRegisters(pAd, i, 2, &LogicalAddress);
			DebugPrint(
				TRACE, DBG_HW_ACCESS,
				"eFusePhysicalReadRegisters() read pool addr=0x%04X, value=0x%X\n",
				i, LogicalAddress);
	...
	DebugPrint(
		TRACE, DBG_HW_ACCESS,
		"eFuseWriteRegisters BlkNum = 0x%02X, EFSROM_AOUT= 0x%02X \n",
		BlkNum, EFSROM_AOUT);
	...
	if ((buffer[(Offset >> 1) % 8] | pData[0]) != pData[0]) {
		...
		// Check if we can find a new one, EFUSE_USAGE_MAP_END is the lastest block
		// point to Block #29 (EFUSE_USAGE_MAP_START ~ EFUSE_USAGE_MAP_END)
		//
		eFusePhysicalReadRegisters(pAd, EFUSE_USAGE_MAP_END, 2,
					   &LogicalAddress);
		if ((LogicalAddress & 0xff) != 0) {
	...
	do {
		...
		if (!bWriteSuccess) {
			...
			DebugPrint(
				TRACE, DBG_HW_ACCESS,
				"!!!Check it, Write Remainder bytes at address 0x%X, pData=0x%X \n",
				InBuf[0], InBuf[2]);
			addr = BlkNum * 0x10;
		...
		// convert the address from 10 to 8 bit ( bit7, 6 = parity and bit5 ~ 0 =
		// bit9~4), and write to logical map entry
		tmpOffset = Offset;
		...
		// Step 5. Compare data if not the same, invalidate the mapping entry, then
		// re-write the data until E-fuse is exhausted
		bWriteSuccess = TRUE;
		for (i = 0; i < 8; i++) {
			addr = BlkNum * 0x10;
		...
		// Step 6. invlidate mapping entry and find a free mapping entry if not
		// succeed
		if (!bWriteSuccess) {
			DebugPrint(TRACE, DBG_HW_ACCESS,
			...
			for (i = EFUSE_USAGE_MAP_START;
			     i <= EFUSE_USAGE_MAP_END; i += 2) {
				eFusePhysicalReadRegisters(pAd, i, 2,
							   &LogicalAddress);
				if ((LogicalAddress & 0xff) == 0) {
					BlkNum = i - EFUSE_USAGE_MAP_START;
					break;
				} else if (((LogicalAddress >> 8) & 0xff) ==
					   0) {
					if (i != EFUSE_USAGE_MAP_END) {
						BlkNum = i + 1 -
		...
		LoopCount++;
	} while ((!bWriteSuccess) && (LoopCount < 200) &&
		 (!RT_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)));

	return STATUS_SUCCESS;
}

NTSTATUS eFuseWrite(IN RTBTH_ADAPTER *pAd, IN USHORT Offset, IN UCHAR *pData,
		    IN USHORT length)
{
	// int i;
	USHORT i;
...
VOID RaiseClock(IN PRTBTH_ADAPTER pAd, IN ULONG *x)
{
	*x = *x | EESK;
...
VOID LowerClock(IN PRTBTH_ADAPTER pAd, IN ULONG *x)
{
	*x = *x & ~EESK;
	RT_IO_WRITE32(pAd, E2PROM_CSR, *x);
	rtbt_usec_delay(1);
...
USHORT ShiftInBits(IN PRTBTH_ADAPTER pAd)
{
	ULONG x, i;
	USHORT data = 0;
...
VOID ShiftOutBits(IN PRTBTH_ADAPTER pAd, IN USHORT data, IN USHORT count)
{
	ULONG x, mask;
...
VOID EEpromCleanup(IN PRTBTH_ADAPTER pAd)
{
	ULONG x;
...
VOID EWEN(IN PRTBTH_ADAPTER pAd)
{
	ULONG x;
...
VOID EWDS(IN PRTBTH_ADAPTER pAd)
{
	ULONG x;
...
int rtbt_prom_read16(IN PRTBTH_ADAPTER pAd, IN USHORT Offset, OUT USHORT *pData)
{
	ULONG x;
...
int rtbt_prom_write16(IN PRTBTH_ADAPTER pAd, IN USHORT Offset, IN USHORT Data)
{
	ULONG x;
...
USHORT Bth_EEPROM_READ16(IN RTBTH_ADAPTER *pAd, IN USHORT Offset)
{
	ULONG value = 0;
	USHORT data;
	LONG count = 0;
	...
	while ((value & 0x80000000) && (count <= 100)) {
		...
		if (value == 0xffffffff) {
			DebugPrint(
				TRACE, DBG_HW_ACCESS,
				"Bth_EEPROM_READ16: Card maybe not exist or bt_func not "
				"enable  bt_fun= %x\n",
				pAd->btFunCtrl.word);
			break;
		}
...
VOID Bth_EEPROM_WRITE16(IN PRTBTH_ADAPTER pAd, IN USHORT Offset, IN USHORT Data)
{
	ULONG value = 0;
	LONG count = 0;
...
VOID dump_mac_reg(IN RTBTH_ADAPTER *pAd)
{
	...
	for (cnt = 0; cnt < sizeof(RT3298_REG_ADDR) / sizeof(UINT32);) {
		...
		DebugPrint(TRACE, DBG_MISC, "0x%x=0x%x\t", RT3298_REG_ADDR[cnt],
			   mac_val);
		if (((++cnt) % 4) == 0)
...
void BthInitializePrerequire(IN RTBTH_ADAPTER *pAd)
{
	...
	DebugPrint(TRACE, DBG_INIT,
		   " Dump MAC registers before call BthEnableBtFunc\n");
	dump_mac_reg(pAd);
	...
	DebugPrint(TRACE, DBG_INIT,
		   " Dump MAC registers After call BthEnableBtFunc\n");
	dump_mac_reg(pAd);
	...
	// INTERNAL_EFFUSE_PROM)?"EFUSE":"EEPROM");
	DebugPrint(TRACE, DBG_INIT, "Use %s\n",
		   (pAd->bUseEfuse == INTERNAL_EFFUSE_PROM) ? "EFUSE" :
							      "EEPROM");
	...
	usValue = (Bth_EEPROM_READ16(pAd, EEPROM_HW_RADIO_SUPPORT) & 0xff00) >>
		  8;
...
int BthInitialize(IN RTBTH_ADAPTER *pAd)
{
	int status;
	...
	if (!NT_SUCCESS(status)) {
		DebugPrint(ERROR, DBG_INIT,
			   "BthInitializeAdapter failed: 0x%x\n", status);
		return status;
	}
...
VOID Rtbth_Set_Radio_Led(IN PRTBTH_ADAPTER pAd, IN BOOLEAN Enable)
{
	ULONG value = 0;
	...
	if (Enable == TRUE) {
		...
		//
		pAd->btFunCtrl.word &=
			~0x01000000; /* GPIO1 output enable: Output 0, input 1 	*/
		...
		DebugPrint(TRACE, DBG_INIT,
			   "Rtbth_Set_Radio_Led ON readback 0x%08X\n", value);
	} else /* Set BT Led OFF */
	{
		...
		RT_IO_FORCE_READ32(pAd, BT_FUN_CTRL, &value);
		DebugPrint(TRACE, DBG_INIT,
			   "Rtbth_Set_Radio_Led Off readback 0x%08X\n", value);
	}
}

VOID BthRadioOff(IN PRTBTH_ADAPTER pAd)
{
	ULONG MacValue;
...
VOID BthRadioOn(IN PRTBTH_ADAPTER pAd)
{
...
int rtbt_dev_ctrl_init(IN struct rtbt_os_ctrl **pDevCtrl, IN void *csr)
{
	struct rtbt_os_ctrl *os_ctrl;
	...
	//	size = sizeof(struct rtbt_dev_ctrl) + sizeof(struct rtbt_os_ctrl);
	////sean wang linux _RTBTH_ADAPTER
	size = sizeof(struct _RTBTH_ADAPTER) +
	       sizeof(struct rtbt_os_ctrl); // sean wang linux
	...
	DebugPrint(TRACE, DBG_INIT,
		   ...
		   (ULONG)os_ctrl, sizeof(struct rtbt_os_ctrl), (ULONG)pAd,
		   sizeof(struct _RTBTH_ADAPTER), size); // sean wang linux
...
#ifdef DBG
	DebugPrint(TRACE, DBG_INIT,
		   "os_ctrl.if_ops.pci_ops.isr=0x%lx(0x%lx), "
	...
	if (spin_status | event_status | timer_status) {
		DebugPrint(ERROR, DBG_INIT,
			   "%s():Init failed(spin=%d, event=%d, timer=%d)\n",
			   __FUNCTION__, spin_status, event_status,
			   timer_status);
...
static struct ral_dev_id rtbt_3298_dev_ids[] = { { 0x1814, 0x3298 }, { 0, 0 } };

static struct rtbt_dev_ops rtbt_3298_dev_ops = {
	.dev_ctrl_init = rtbt_dev_ctrl_init,
	.dev_ctrl_deinit = rtbt_dev_ctrl_deinit,
	.dev_resource_init =
		rtbt_dev_resource_init, /* rtbt_pci_resource_init */
	.dev_resource_deinit =
		rtbt_dev_resource_deinit, /* rtbt_pci_resource_deinit */
...
VOID BthShutdown(IN PRTBTH_ADAPTER pAd)
{
...
INT rtbt_hps_open(void *pDevCtrl)
{
	...
	pAd->sco_event_buf = ral_mem_alloc(128 * 4, RAL_MEM_NORMAL);
	pAd->sco_data_buf =
		ral_mem_alloc(PKT_HV3_MAX_DATA_LEN * 2 + 3, RAL_MEM_NORMAL);
	...
	DebugPrint(TRACE, DBG_INIT, "BthEnableInterrupt \n");
	//	DebugPrint(TRACE, DBG_THREAD, "Shiang: Here we stop the
	//rtbt_hps_open!Remove it latter(status=%d)!\n", status); 	return status;
	BthEnableInterrupt(pAd);
	BthEnableRxTx(pAd);
...
int rtbth_rx_packet(void *pdata, RXBI_STRUC rxbi, void *buf, unsigned int len)
{
	...
	unsigned int sz_need = 2 + sizeof(rxbi) + sizeof(len) + len;
	int ret = 0;
	unsigned long cpuflags;

	DebugPrint(TRACE, DBG_INIT,
		   "\n **sz_need for rx=%d, kfifo_avail(rx)=%d\n", sz_need,
		   kfifo_avail(gpAd->rx_fifo));

	DebugPrint(
		TRACE, DBG_INIT,
		"#########Rx Data len = %d, rxbi.len=%d, rxbi.crc = %d, rxbi.ptt=%d \n",
	...
	DebugPrint(TRACE, DBG_INIT,
		   "\n **after rx data in , kfifo_len (%d), sz_need(%d), "
		   "sizeof(rxbi)=%d, sizeof(len)=%d \n",
		   kfifo_len(gpAd->rx_fifo), sz_need, sizeof(rxbi),
		   sizeof(len));
