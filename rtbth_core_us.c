 ...
 *
 */
_osal_inline_ int osal_thread_create(P_OSAL_THREAD pThread)
{
...
_osal_inline_ int osal_thread_run(P_OSAL_THREAD pThread)
{
	if (pThread->pThread) {
...
_osal_inline_ int osal_thread_stop(P_OSAL_THREAD pThread)
{
	int iRet;
...
_osal_inline_ int osal_thread_should_stop(P_OSAL_THREAD pThread)
{
	if ((pThread) && (pThread->pThread)) {
...
_osal_inline_ int osal_thread_wait_for_event(P_OSAL_THREAD pThread,
					     P_OSAL_EVENT pEvent,
...
_osal_inline_ int osal_thread_destroy(P_OSAL_THREAD pThread)
{
	if (pThread && (pThread->pThread)) {
...
_osal_inline_ int osal_signal_init(P_OSAL_SIGNAL pSignal)
{
	if (pSignal) {
...
_osal_inline_ int osal_wait_for_signal(P_OSAL_SIGNAL pSignal)
{
	if (pSignal) {
...
_osal_inline_ int osal_wait_for_signal_timeout(P_OSAL_SIGNAL pSignal)
{
	/* return wait_for_completion_interruptible_timeout(&pSignal->comp,
   ...
   */
	return wait_for_completion_timeout(
		&pSignal->comp, msecs_to_jiffies(pSignal->timeoutValue));
}

_osal_inline_ int osal_raise_signal(P_OSAL_SIGNAL pSignal)
{
...
_osal_inline_ int osal_signal_deinit(P_OSAL_SIGNAL pSignal)
{
	// TODO:[FixMe][GeorgeKuo]: DO sanity check here!!!
...
int osal_wait_for_event(P_OSAL_EVENT pEvent, int (*condition)(PVOID),
			void *cond_pa)
{
	return wait_event_interruptible(pEvent->waitQueue, condition(cond_pa));
}

int osal_wait_for_event_timeout(P_OSAL_EVENT pEvent, int (*condition)(PVOID),
				void *cond_pa)
{
	return wait_event_interruptible_timeout(
		pEvent->waitQueue, condition(cond_pa),
...
static int _rtbth_us_handler(RTBTH_ADAPTER *pAd, P_STP_OP pStpOp)
{
	...
	case RTBTH_TEST_EVENT:
		DebugPrint(
			TRACE, DBG_INIT,
			"Event =%d, jiffies = %d, evt_fifo_len = %d, in=%d\n",
		...
		if (kfifo_avail(pAd->evt_fifo) >= sizeof(RTBTH_EVENT_T)) {
			...
			DebugPrint(TRACE, DBG_INIT,
				   "evt_fifo size is not available!\n");
			ret = -1;
		}
		break;
	...
	case INIT_EPINIT_EVENT:
		...
		//  msleep(1000);
		sync_wq_flag = 0;
		ret = wait_event_interruptible_timeout(sync_wq,
						       sync_wq_flag != 0, HZ);
		if (!ret)
...
static P_OSAL_OP _rtbth_us_get_op(RTBTH_ADAPTER *pAd, P_OSAL_OP_Q pOpQ)
{
	P_OSAL_OP pOp;
...
static int _rtbth_us_wait_for_evt(void *pvData)
{
	...
	DebugPrint(LOUD, DBG_INIT, "%s: pAd->rActiveOpQ = %d\n", __func__,
		   RB_COUNT(&pAd->rActiveOpQ));

	return ((!RB_EMPTY(&pAd->rActiveOpQ)) ||
		osal_thread_should_stop(&pAd->PSMd));
}
...
static int _rtbth_us_proc(void *pvData)
{
	...
	DebugPrint(
		TRACE, DBG_INIT,
		"rtbth_us starts running: pWmtDev(0x%p) [pol, rt_pri, n_pri, "
		"pri]=[%d, %d, %d, %d] \n",
		pAd, current->policy, current->rt_priority,
		current->normal_prio, current->prio);

	for (;;) {
		pOp = NULL;

		osal_wait_for_event(&pAd->STPd_event, _rtbth_us_wait_for_evt,
				    (void *)pAd);
...
static int rtbth_us_open(struct inode *inode, struct file *file)
{
	...
	if (!atomic_dec_and_test(&rtbth_us_avail)) {
		...
		DebugPrint(WARNING, DBG_INIT,
			   "rtbth occupied: rtbth_us_avail=%d\n",
...
ssize_t rtbth_us_read(struct file *filp, char __user *buf, size_t count,
		      loff_t *f_pos)
{
	ssize_t retval = 0;
	RTBTH_EVENT_T evt = 0;
	...
	while (1) {
		if (kfifo_len(gpAd->evt_fifo) > 0) {
			...
			if (pollm) {
				...
				DebugPrint(
					TRACE, DBG_INIT,
					"%s: wait for the event, pending on kernel space\n",
					__func__);
				wait_ret = wait_event_interruptible_timeout(
					evt_wq, evt_wq_flag != 0, HZ);
				if (wait_ret) {
					if (-ERESTARTSYS == wait_ret) {
						DebugPrint(
							TRACE, DBG_INIT,
							"%s: signaled by -ERESTARTSYS(%ld) \n ",
							__func__, wait_ret);
						return -EAGAIN;
					} else {
						DebugPrint(
				...
				DebugPrint(
					TRACE, DBG_INIT,
					"%s: receive the event, return to user space\n",
					__func__);
			}
		}
	}
		      ...
		      loff_t *f_pos)
{
	...
	while (1) {
		if (kfifo_len(gpAd->evt_fifo) > 0) {
			...
			DebugPrint(
				TRACE, DBG_INIT,
				"%s: remaining event fifo len = %d, out=%d\n",
				__func__, kfifo_len(gpAd->evt_fifo), ++out);
			break;
		} else {
			DebugPrint(
				TRACE, DBG_INIT,
				"%s: wait for the event, pending on kernel space, %d, "
				"kfifo_len(evt)=%d\n",
...
ssize_t rtbth_us_write(struct file *filp, const char __user *buf, size_t count,
		       loff_t *f_pos)
{
	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;
...
VOID MCU_Read_Memory(IN PRTBTH_ADAPTER pAd, IN UINT32 Address,
		     OUT PUINT8 pBuffer, IN UINT16 Length)
{
	INT32 i, j, k;
	UINT32 Val = 0;
...
VOID MCU_Write_Memory(IN PRTBTH_ADAPTER pAd, IN UINT32 Address,
		      IN PUINT8 pBuffer, IN UINT16 Length)
{
	INT32 i, j, k;
	UINT32 Val;
...
VOID PDMA_Transmit_TxRing(IN PRTBTH_ADAPTER pAd, IN UINT8 LLIdx,
			  IN UINT8 RingIdx, IN UINT8 PktType, IN UINT8 Llid,
			  IN UINT8 PFlow, IN UINT16 Tag, IN PUINT8 pPacket,
			  IN UINT16 Length, IN BOOLEAN AutoFlushable,
			  IN UINT8 PktSeq, IN UINT8 ChSel, IN UINT8 CodeType)
{
	PUCHAR pData;
	ULONG TxCpuIdx;
	PTXD_STRUC pTxD;
	...
	pTxD->QSEL = RingIdx;
	pTxD->SDLen0 =
		((sizeof(TXBI_STRUC) + Length + 3) / 4) * 4; // Word boundary
	pTxD->SDLen1 = 0;
	...
	// pTxBI->ChSel = ChSel;
	pTxBI->ChSel =
		(LLIdx >= SYNC_LOGICAL_LINK && LLIdx < ACLU_LOGICAL_LINK) ?
			(LLIdx - SYNC_LOGICAL_LINK) :
			ChSel;
...
VOID PDMA_Reset_TxRing(IN PRTBTH_ADAPTER pAd, IN UINT8 TxRingIdx)
{
	UINT8 Index, Count = 0;
	//   KIRQL                   oldIrql = 0;
	...
	if (TxRingIdx >= NUM_OF_TX_RING) {
		DebugPrint(ERROR, DBG_HW_ACCESS,
			   "PDMA_Reset_TxRing: Erorr ! TxRingIdx %d\n",
			   TxRingIdx);
		return;
	}
	...
	do {
		...
		if (GloCfg.field.TxDMABusy == 0) {
			...
			// RT_IO_WRITE32(pAd, TX_MAX_CNT0 + TxRingIdx * RINGREG_DIFF,
			// TX_RING_SIZE);
			RT_IO_WRITE32(pAd,
				      TX_MAX_CNT0 + TxRingIdx * RINGREG_DIFF,
				      TxRingSize);
			RT_IO_WRITE32(
				pAd, TX_CTX_IDX0 + TxRingIdx * RINGREG_DIFF, 0);
	...
	if (GloCfg.field.TxDMABusy) {
		DebugPrint(WARNING, DBG_HW_ACCESS,
			   "PDMA_Reset_TxRing: Warning ! TxDMA busy(0x%08x)\n",
			   GloCfg.word);
	}
}

BOOLEAN
PDMA_Is_TxRing_Empty(IN PRTBTH_ADAPTER pAd, IN UINT8 TxRingIdx)
{
	...
	if (TxRingIdx >= NUM_OF_TX_RING) {
		DebugPrint(ERROR, DBG_HW_ACCESS,
			   "PDMA_Check_TxRing_Status: Erorr ! TxRingIdx %d\n",
			   TxRingIdx);
		return TRUE;
	}
	...
	if (TxCpuIdx != TxDmaIdx) {
		DebugPrint(
			WARNING, DBG_HW_ACCESS,
			"PDMA_Check_TxRing_Status: Warning ! TxRing %d, CpuIdx %d, "
			"DmaIdx %d, SwFreeIdx %d\n",
	...
	if ((QueueEmptyStatus & (1 << TxRingIdx)) == 0) {
		...
		DebugPrint(
			WARNING, DBG_HW_ACCESS,
			"PDMA_Check_TxRing_Status: Warning ! TxRing %d, Queue not "
			"empty(0x%04x)\n",
...
ULONG
PDMA_Get_Txring_Freeno(IN PRTBTH_ADAPTER pAd, IN UINT8 RingIdx)
{
	UINT8 TxRingSize = BthGetTxRingSize(pAd, RingIdx);

	return (pAd->TxRing[RingIdx].TxSwFreeIdx >
		pAd->TxRing[RingIdx].TxCpuIdx) ?
		       (pAd->TxRing[RingIdx].TxSwFreeIdx -
			pAd->TxRing[RingIdx].TxCpuIdx - 1) :
		       (pAd->TxRing[RingIdx].TxSwFreeIdx + TxRingSize -
			pAd->TxRing[RingIdx].TxCpuIdx - 1);
}
...
void rtbth_us_mcu_write(unsigned long addr, unsigned char *buf,
			unsigned short len)
{
	MCU_Write_Memory(gpAd, addr, buf, len);
}

void rtbth_us_mcu_read(unsigned long addr, unsigned char *buf,
		       unsigned short len)
{
...
{
	PDMA_Transmit_TxRing(gpAd, tx_ctrl->ll_idx, tx_ctrl->ring_idx,
			     tx_ctrl->pkt_type, tx_ctrl->ll_id, tx_ctrl->pflow,
			     tx_ctrl->tag, buf /*tx_ctrl->ppkt*/, tx_ctrl->len,
			     tx_ctrl->auto_flushable, tx_ctrl->pkt_seq,
			     tx_ctrl->ch_sel, tx_ctrl->code_type);
}
...
int _rtbt_hci_dev_receive(void *bt_dev, int pkt_type, unsigned char *buf,
			  int len)
{
	return rtbt_hci_dev_receive(bt_dev, pkt_type, buf, len);
}

long rtbth_us_unlocked_ioctl(struct file *filp, unsigned int cmd,
			     unsigned long arg)
{
	int retval = 0;
	...
	case RTBTH_IOCPCI32WRITE:
		...
		do {
			...
			if (copy_from_user(&pci32wr, (void *)arg,
					   sizeof(pci32wr))) {
				retval = -EFAULT;
			...
			//  DebugPrint(ERROR, DBG_INIT,"RTBTH_IOCPCI32WRITE: addr =0x%08x, value =
	...
	case RTBTH_IOCPCI32READ:
		...
		do {
			...
			if (copy_from_user(&pci32rd, (void *)arg,
					   sizeof(pci32rd))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			if (copy_to_user(
				    (void *)(arg +
					     offsetof(struct rtbth_pci32read,
						      value)),
				    &val, sizeof(val))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_to_user failed at %d\n",
					   __LINE__);
				break;
			}
		} while (0);
	...
	case RTBTH_IOCMCUWRITE:
		...
		do {
			...
			if (copy_from_user(&mcuwr, (void *)arg,
					   sizeof(mcuwr))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			DebugPrint(
				TRACE, DBG_INIT,
				"RTBTH_IOCMCUWRITE: addr =0x%08x, len = %d\n",
				mcuwr.addr, mcuwr.len);

			if (mcuwr.len > 2048) {
				DebugPrint(
					ERROR, DBG_INIT,
					"RTBTH_IOCMCUWRITE: mcuwr.len > 2048\n");
				retval = -EFAULT;
				break;
			}

			if (copy_from_user(&buf[0], (void *)(mcuwr.buf),
					   mcuwr.len)) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			DebugPrint(
				TRACE, DBG_INIT,
				"RTBTH_IOCMCUWRITE: [0]=0x%02x [1]=0x%02x [2]=0x%02x "
				"[3]=0x%02x [4]=0x%02x [5]=0x%02x \n",
	...
	case RTBTH_IOCMCUREAD:
		...
		do {
			...
			if (copy_from_user(&mcurd, (void *)arg,
					   sizeof(mcurd))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			DebugPrint(TRACE, DBG_INIT,
				   "RTBTH_IOCMCUREAD: addr =0x%08x, len = %d\n",
				   mcurd.addr, mcurd.len);

			if (mcurd.len > 2048) {
				DebugPrint(
					ERROR, DBG_INIT,
					"RTBTH_IOCMCUREAD: mcuwr.len > 2048\n");
				retval = -EFAULT;
				break;
			}
			...
			if (copy_to_user((void *)(mcurd.buf), &buf[0],
					 mcurd.len)) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_to_user failed at %d\n",
	...
	case RTBTH_IOCBBREAD:
		...
		do {
			...
			if (copy_to_user(
				    (void *)(arg + offsetof(struct rtbth_bbread,
							    val)),
				    &val, sizeof(val))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
	...
	case RTBTH_IOCRFREAD:
		...
		do {
			...
			if (copy_to_user(
				    (void *)(arg + offsetof(struct rtbth_rfread,
							    val)),
				    &val, sizeof(val))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
	...
	case RTBTH_IOCBZWRITE:
		...
		do {
			...
			if (copy_from_user(&bzwr, (void *)arg,
					   sizeof(struct rtbth_bzwrite))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
					   __LINE__);
			...
			DebugPrint(TRACE, DBG_INIT,
				   "RTBTH_IOCBZWRITE: type = %d, len = %d\n",
				   bzwr.type, bzwr.len);

			if (copy_from_user(&buf, (void *)(bzwr.buf),
					   bzwr.len)) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			// uslm todo: sean wang
			_rtbt_hci_dev_receive(gpAd, bzwr.type, &buf[0],
					      bzwr.len);
	...
	case RTBTH_IOCBZREAD:
		...
		do {
			...
			if (copy_from_user(&bzread, (void *)arg,
					   sizeof(bzread))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			DebugPrint(
				TRACE, DBG_INIT,
				"bz type = %d, len of fifo = %d, len of buf = %d\n",
				bzread.type, kfifo_len(fifo), bzread.len_buf);
			if (kfifo_len(fifo) >= sz_need) {
				RtlAssert(kfifo_out(fifo, &chk[0],
						    sizeof(chk)) > 1);
				if (chk[0] != 0xcc && chk[1] != 0xcc) {
					DebugPrint(
						ERROR, DBG_INIT,
						"@@@@read bz data err: delimeter parsing fail, "
						"chk[0]=0x%02x, chk[1]=0x%02x\n",
						chk[0], chk[1]);
					retval = -EFAULT;
					spin_unlock_irqrestore(fifo_sp,
							       cpuflags);
					break;
				}
				RtlAssert(kfifo_out(fifo, &len,
						    sizeof(unsigned long)) > 1);
				if (len > 2048 || len < 1) {
					DebugPrint(
						ERROR, DBG_INIT,
						"@@@@read bz data err: len invalid (%d)\n",
						len);
					retval = -EFAULT;
					spin_unlock_irqrestore(fifo_sp,
							       cpuflags);
				...
				RtlAssert(kfifo_out(fifo, buf, len) > 1);
			} else {
				DebugPrint(
					ERROR, DBG_INIT,
					"@@@@read bz data err: data not available, kfifo_len (%d), "
					"sz_need = %d\n",
			...
			if (copy_to_user((void *)(arg + sizeof(int) +
			...
			if (copy_from_user(
				    &buf_addr,
				    (void *)(arg + offsetof(struct rtbth_bzread,
							    buf)),
	...
	case RTBTH_IOCTRCTRL:
		...
		do {
			...
			if (copy_from_user(&trctl, (void *)arg,
					   sizeof(trctl))) {
				retval = -EFAULT;
				DebugPrint(ERROR, DBG_INIT,
					   "copy_from_user failed at %d\n",
			...
			DebugPrint(
				TRACE, DBG_INIT,
				"RTBTH_IOCTRCTRL: op = %d, len_struct = %d\n",
			...
			if (trctl.op == tr_tx) {
				...
				if (copy_from_user(&tx_ctrl,
						   (void *)(trctl.payload),
						   sizeof(tx_ctrl))) {
					retval = -EFAULT;
					DebugPrint(
						ERROR, DBG_INIT,
						"copy_from_user failed at %d, sizeof(tx_ctrl)=%d, "
						"len_struct=%d\n",
						__LINE__, sizeof(tx_ctrl),
						sizeof(len_struct));
					break;
				}

				if (copy_from_user(&buf[0],
						   (void *)(tx_ctrl.ppkt),
						   tx_ctrl.len)) {
					retval = -EFAULT;
					DebugPrint(
				...
				if (copy_from_user(&pempty,
						   (void *)(trctl.payload),
						   sizeof(pempty))) {
					retval = -EFAULT;
					DebugPrint(
						ERROR, DBG_INIT,
						"111copy_from_user failed at %d\n",
						__LINE__);
					break;
				}
				...
				DebugPrint(
					TRACE, DBG_INIT,
					"pempty.ring_idx = %d, offsetof(struct "
					"rtbth_tx_empty,isempty)=%d\n",
					pempty.ring_idx,
					offsetof(struct rtbth_tx_empty,
						 isempty));

#if 1
				isempty = rtbth_us_txring_isempty(
					pempty.ring_idx);
#else
				isempty = 1;
#endif
				...
				if (copy_to_user(
					    (void *)(trctl.payload +
						     offsetof(
							     ...
							     isempty)),
					    &isempty, sizeof(isempty))) {
					retval = -EFAULT;
					DebugPrint(
						ERROR, DBG_INIT,
						"222copy_to_user failed at %d\n",
						__LINE__);
					break;
				}
				...
				DebugPrint(
					...
					"rtbth_tx_empty,isempty)=%d\n",
					pfrcnt.ring_idx,
					offsetof(struct rtbth_tx_frcnt,
						 free_cnt));

#if 1
				frcnt = rtbth_us_txring_free_cnt(
					pfrcnt.ring_idx);
#else
				frcnt = 0xdd;
#endif
				...
				if (chk[0] != 0xcc && chk[1] != 0xcc) {
					DebugPrint(
						ERROR, DBG_INIT,
						"@@@@rx packet: delimeter parsing fail, chk[0]=0x%02x, "
						"chk[1]=0x%02x\n",
						chk[0], chk[1]);
					retval = -EFAULT;
					spin_unlock_irqrestore(fifo_sp,
				...
				RtlAssert(kfifo_out(fifo, &rxbi, sizeof(rxbi)) >
					  1);
				RtlAssert(kfifo_out(fifo, &pkt_len,
						    sizeof(pkt_len)) > 1);
				// if(pkt_len > 2048 || pkt_len < sz_need){
				if (pkt_len > 2048) {
					DebugPrint(
						ERROR, DBG_INIT,
						"@@@@rx packet err: len invalid (%d)\n",
						pkt_len);
					retval = -EFAULT;
					spin_unlock_irqrestore(fifo_sp,
							       cpuflags);
				...
				DebugPrint(
					TRACE, DBG_INIT,
					"op == tr_rx_packet, rxbi.Len=0x%x, rxbi.ScoSeq = 0x%x, "
					"pkt_len=%d, buf[0]=0x%x, buf[1]=0x%x, buf[2]=0x%x, "
					"buf[3]=0x%x, buf[4]=0x%x\n",
					rxbi.Len, rxbi.ScoSeq, pkt_len, buf[0],
				...
				if (copy_to_user((void *)(buf_addr), &buf,
						 pkt_len)) {
					retval = -EFAULT;
					DebugPrint(
						ERROR, DBG_INIT,
...
static struct file_operations rtbth_us_fops = { .open = rtbth_us_open,
						.release = rtbth_us_close,
						.read = rtbth_us_read,
						.write = rtbth_us_write,
						.unlocked_ioctl =
							rtbth_us_unlocked_ioctl,
						.poll = rtbth_us_poll };
...
#define RTBTH_US_THREAD_NAME "rtbth_us"
	memcpy(pAd->PSMd.threadName, RTBTH_US_THREAD_NAME,
	       strlen(RTBTH_US_THREAD_NAME));
