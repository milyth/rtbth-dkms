...
_osal_inline_ int osal_thread_create(P_OSAL_THREAD pThread) {
  pThread->pThread = kthread_create(pThread->pThreadFunc, pThread->pThreadData,
                                    pThread->threadName);
  ...
  if ((pThread) && (pThread->pThread) && (pEvent) && (pChecker)) {
    ...
    return wait_event_interruptible(
        ...
        (/*!RB_EMPTY(&pDevWmt->rActiveOpQ) ||*/
         osal_thread_should_stop(pThread) || (*pChecker)(pThread)));
  }
  return -1;
}
...
int osal_event_deinit(P_OSAL_EVENT pEvent) { return 0; }
...
static int _rtbth_us_put_op(RTBTH_ADAPTER *pAd, P_OSAL_OP_Q pOpQ,
                            P_OSAL_OP pOp) {
  int ret;
...
static int _rtbth_us_proc(void *pvData) {
  ...
  DebugPrint(TRACE, DBG_INIT,
             ...
             "pri]=[%d, %d, %d, %d] \n",
             pAd, current->policy, current->rt_priority, current->normal_prio,
             current->prio);

  for (;;) {
    pOp = NULL;

    osal_wait_for_event(&pAd->STPd_event, _rtbth_us_wait_for_evt, (void *)pAd);
...
ssize_t rtbth_us_read(struct file *filp, char __user *buf, size_t count,
  ...
  while (1) {
    if (kfifo_len(gpAd->evt_fifo) > 0) {
      RtlAssert(kfifo_out(gpAd->evt_fifo, &evt, sizeof(RTBTH_EVENT_T)) > 0);
      DebugPrint(TRACE, DBG_INIT, "%s: event fifo len = %d\n", __func__,
                 kfifo_len(gpAd->evt_fifo));
      break;
    } else {
      if (pollm) {
        retval = -EAGAIN;
        ...
        DebugPrint(TRACE, DBG_INIT,
                   ...
                   __func__);
        wait_ret =
            wait_event_interruptible_timeout(evt_wq, evt_wq_flag != 0, HZ);
        if (wait_ret) {
          ...
          DebugPrint(NOISY, DBG_INIT, "%s: wait timeout %d\n ", __func__,
                     wait_ret);
          retval = -EFAULT;
          goto out;
        }
...
ssize_t rtbth_us_read(struct file *filp, char __user *buf, size_t count,
  ...
  while (1) {
    if (kfifo_len(gpAd->evt_fifo) > 0) {
      RtlAssert(kfifo_out(gpAd->evt_fifo, &evt, sizeof(RTBTH_EVENT_T)) > 1);
...
VOID MCU_Write_Memory(IN PRTBTH_ADAPTER pAd, IN UINT32 Address,
  ...
  k = Length % 4;
  for (j = 0; j < i; j++) {
    Val = ((UINT32)pBuffer[j * 4]) | (((UINT32)pBuffer[(j * 4) + 1]) << 8) |
...
VOID PDMA_Transmit_TxRing(IN PRTBTH_ADAPTER pAd, IN UINT8 LLIdx,
  ...
  // pTxBI->ChSel = ChSel;
  pTxBI->ChSel = (LLIdx >= SYNC_LOGICAL_LINK && LLIdx < ACLU_LOGICAL_LINK)
  ...
  RT_IO_WRITE32(pAd, TX_CTX_IDX0 + RingIdx * RINGREG_DIFF, pTxRing->TxCpuIdx);
...
long rtbth_us_unlocked_ioctl(struct file *filp, unsigned int cmd,
  ...
  case RTBTH_IOCPCI32READ:
    ...
    do {
      ...
      if (copy_to_user((void *)(arg + offsetof(struct rtbth_pci32read, value)),
                       &val, sizeof(val))) {
        retval = -EFAULT;
  ...
  case RTBTH_IOCBBREAD:
    ...
    do {
      ...
      if (copy_to_user((void *)(arg + offsetof(struct rtbth_bbread, val)), &val,
                       sizeof(val))) {
        retval = -EFAULT;
        DebugPrint(ERROR, DBG_INIT, "copy_to_user failed at %d\n", __LINE__);
        break;
  ...
  case RTBTH_IOCRFREAD:
    ...
    do {
      ...
      if (copy_to_user((void *)(arg + offsetof(struct rtbth_rfread, val)), &val,
                       sizeof(val))) {
        retval = -EFAULT;
        DebugPrint(ERROR, DBG_INIT, "copy_to_user failed at %d\n", __LINE__);
        break;
  ...
  case RTBTH_IOCBZREAD:
    ...
    do {
      ...
      if (kfifo_len(fifo) >= sz_need) {
        RtlAssert(kfifo_out(fifo, &chk[0], sizeof(chk)) > 1);
        ...
        RtlAssert(kfifo_out(fifo, &len, sizeof(unsigned long)) > 1);
        if (len > 2048 || len < 1) {
          DebugPrint(ERROR, DBG_INIT,
      ...
      if (copy_to_user((void *)(arg + sizeof(int) + sizeof(unsigned short)),
                       &len, sizeof(unsigned short))) {
        retval = -EFAULT;
        break;
      }

      if (copy_from_user(&buf_addr,
                         (void *)(arg + offsetof(struct rtbth_bzread, buf)),
  ...
  case RTBTH_IOCTRCTRL:
    ...
    do {
      ...
      if (trctl.op == tr_tx) {
        ...
        if (copy_from_user(&tx_ctrl, (void *)(trctl.payload),
                           sizeof(tx_ctrl))) {
          retval = -EFAULT;
          DebugPrint(ERROR, DBG_INIT,
        ...
        if (copy_to_user((void *)(trctl.payload +
                                  offsetof(struct rtbth_tx_empty, isempty)),
        ...
        if (copy_to_user((void *)(trctl.payload +
                                  offsetof(struct rtbth_tx_frcnt, free_cnt)),
                         &frcnt, sizeof(frcnt))) {
          retval = -EFAULT;
          DebugPrint(ERROR, DBG_INIT, "copy_to_user failed at %d\n", __LINE__);
          break;
        ...
        RtlAssert(kfifo_out(fifo, &rxbi, sizeof(rxbi)) > 1);
        RtlAssert(kfifo_out(fifo, &pkt_len, sizeof(pkt_len)) > 1);
        ...
        DebugPrint(TRACE, DBG_INIT,
                   ...
                   rxbi.Len, rxbi.ScoSeq, pkt_len, buf[0], buf[1], buf[2],
                   buf[3], buf[4]);

        if (copy_to_user(
                (void *)(trctl.payload + offsetof(struct rtbth_rx_ctrl, rxbi)),
                &rxbi, sizeof(rxbi))) {
          retval = -EFAULT;
          DebugPrint(ERROR, DBG_INIT, "copy_to_user failed at %d\n", __LINE__);
          break;
        ...
        idx = 0;
        if (copy_to_user(
                (void *)(trctl.payload + offsetof(struct rtbth_rx_ctrl, rdidx)),
                &idx, sizeof(idx))) {
          retval = -EFAULT;
          DebugPrint(ERROR, DBG_INIT, "copy_to_user failed at %d\n", __LINE__);
          break;
        ...
        if (copy_to_user((void *)(trctl.payload +
                                  offsetof(struct rtbth_rx_ctrl, len_pkt)),
        ...
        if (copy_from_user(
                &buf_addr,
                (void *)(trctl.payload + offsetof(struct rtbth_rx_ctrl, pkt)),
                sizeof(unsigned char *))) {
          retval = -EFAULT;
          DebugPrint(ERROR, DBG_INIT, "copy_from_user failed at %d\n",
                     __LINE__);
...
//               mod_timer(&gpAd->tst_evt_timer_sco, jiffies + (10)/(1000/HZ));
  ...
  case RTBTH_IOCDMAC:
    do {
      ...
      if (dmac.dmac_op > 2) {
        DebugPrint(ERROR, DBG_INIT, "No such dmac op=%d\n", dmac.dmac_op);
      }
    } while (0);
    break;
#if 1
