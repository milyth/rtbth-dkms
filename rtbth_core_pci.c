...
#endif
{
	...
	if (hdev) {
		...
		if (os_ctrl && os_ctrl->if_ops.pci_ops.isr) {
			retval = (os_ctrl->if_ops.pci_ops.isr)(
				os_ctrl->dev_ctrl);
		} else {
			if (os_ctrl)
				BT_ERR("os_ctrl->if_ops.pci_ops.isr=0x%p",
...
#endif
	if (retval != 0)
		BT_ERR("RT_BT: request_irq (IRQ=%d) ERROR(%d)", pdev->irq,
		       retval);
	else
		BT_WARN("%s(): request_irq (IRQ=%d)done, isr_handler=0x%lx!",
...
int rtbt_dma_mem_alloc(IN void *if_dev, IN unsigned long len,
		       IN unsigned char cached, OUT void **virt_addr,
		       OUT PHYSICAL_ADDRESS *phy_addr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0)
	*virt_addr = (PVOID)dma_alloc_coherent(&((struct pci_dev *)if_dev)->dev,
					       sizeof(char) * len,
					       (dma_addr_t *)phy_addr,
					       GFP_KERNEL);
#else
...
void rtbt_dma_mem_free(IN void *if_dev, IN unsigned long len,
		       IN void *virt_addr, IN PHYSICAL_ADDRESS phy_addr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0)
	dma_free_coherent(&((struct pci_dev *)if_dev)->dev, len, virt_addr,
			  phy_addr);
#else
...
loop:
	...
	while (pDevIDList) {
		BT_WARN("RTBT_Tb: vendor=0x%x, device=0x%x", pDevIDList->vid,
			pDevIDList->pid);
		if (pDevIDList->pid && pDevIDList->vid) {
			if (phase == 1)
				...
				pTbPtr->driver_data =
					(unsigned long)pRalDevList->dev_ops;
				BT_WARN("Convert: vendor=0x%x, device=0x%x",
	...
	if (phase == 1) {
		if (size > 0) {
			...
			BT_WARN("DynamicAlloc pci_device_id table at 0x%p with size %d",
				pIdTable, size);
			pTbPtr = pIdTable;
	...
	if (pRalDevList->os_private) {
		...
		while (pTbPtr->vendor != 0) {
			BT_WARN("pci_device_id: vendor=0x%x, device=0x%x",
				pTbPtr->vendor, pTbPtr->device);
			pTbPtr++;
		}
	}
...
UINT8
BthGetTxRingSize(__in PRTBTH_ADAPTER pAd, __in UINT8 RingIdx)
{
...
UINT16
BthGetTxRingPacketSize(IN PRTBTH_ADAPTER pAd, IN UINT8 RingIdx)
{
	UNREFERENCED_PARAMETER(pAd);
	...
	default:
		DebugPrint(ERROR, DBG_INIT,
			   "BthGetTxRingPacketSize: Wrong RingIdx %d\n",
			   RingIdx);
		return 0;
	}
}

UINT32 BthGetTxRingOffset(IN PRTBTH_ADAPTER pAd, IN UINT8 RingIdx)
{
	UNREFERENCED_PARAMETER(pAd);
	...
	case TX_RING_SYNC_IDX + 1:
		return (TX_SYNC_RING_BASE_PTR0_OFFSET_BASE +
			(TX_SYNC_RING_SIZE * TXD_SIZE));
	case TX_RING_SYNC_IDX + 2:
		return (TX_SYNC_RING_BASE_PTR0_OFFSET_BASE +
			2 * (TX_SYNC_RING_SIZE * TXD_SIZE));

	case TX_RING_ACLU_IDX:
		return TX_ACLU_RING_BASE_PTR0_OFFSET_BASE;
	case TX_RING_ACLU_IDX + 1:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			(TX_ACLU_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLU_IDX + 2:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			2 * (TX_ACLU_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLU_IDX + 3:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			3 * (TX_ACLU_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLU_IDX + 4:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			4 * (TX_ACLU_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLU_IDX + 5:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			5 * (TX_ACLU_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLU_IDX + 6:
		return (TX_ACLU_RING_BASE_PTR0_OFFSET_BASE +
			6 * (TX_ACLU_RING_SIZE * TXD_SIZE));

	case TX_RING_ACLC_IDX:
		return TX_ACLC_RING_BASE_PTR0_OFFSET_BASE;
	case TX_RING_ACLC_IDX + 1:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			(TX_ACLC_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLC_IDX + 2:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			2 * (TX_ACLC_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLC_IDX + 3:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			3 * (TX_ACLC_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLC_IDX + 4:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			4 * (TX_ACLC_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLC_IDX + 5:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			5 * (TX_ACLC_RING_SIZE * TXD_SIZE));
	case TX_RING_ACLC_IDX + 6:
		return (TX_ACLC_RING_BASE_PTR0_OFFSET_BASE +
			6 * (TX_ACLC_RING_SIZE * TXD_SIZE));

	case TX_RING_LEACL_IDX:
		return TX_LEACL_RING_BASE_PTR0_OFFSET_BASE;
	case TX_RING_LEACL_IDX + 1:
		return (TX_LEACL_RING_BASE_PTR0_OFFSET_BASE +
			(TX_LEACL_RING_SIZE * TXD_SIZE));

	default:
		DebugPrint(ERROR, DBG_INIT,
			   "BthGetTxRingOffset: Wrong idx(%d)\n", RingIdx);
		return 0;
	}
}

BOOLEAN BthEnableInterrupt(IN RTBTH_ADAPTER *pAd)
{
  ...
  i, temp)); if (temp != DELAYINTMASK) RT_IO_WRITE32(FdoData, INT_MASK_CSR,
  DELAYINTMASK);
...
VOID BthDisableInterrupt(IN struct _RTBTH_ADAPTER *pAd) // sean wang
{
	ULONG value;
	value = 0x0;
...
NTSTATUS BthInitTxRingByIdx(IN PRTBTH_ADAPTER pAd, IN UINT8 RingIdx,
			    IN PVOID RingBaseVa, IN ULONG RingBasePaHigh,
			    ...
			    IN PUINT32 pPerRingAllocBuffSz)
{
	...
	if (RingIdx > 0) {
		...
		RT_IO_WRITE32(pAd, TX_BASE_PTR0 + RingIdx * RINGREG_DIFF,
			      TxRingOffset);
	} else {
		RT_IO_WRITE32(pAd, TX_BASE_PTR0 + RingIdx * RINGREG_DIFF,
			      RingBasePaLow);
	...
	DebugPrint(TRACE, DBG_INIT,
		   "TX_Base_PTR(%02d)-regAddr:%x, regVal=0x%x\n", RingIdx,
		   TX_BASE_PTR0 + RingIdx * RINGREG_DIFF, RingBasePaLow);
	...
	for (Index = 0; Index < TxRingSize; Index++) {
		...
		rtbt_dma_mem_alloc(os_ctrl->if_dev, pDmaBuf->AllocSize, FALSE,
				   &pDmaBuf->AllocVa, &pDmaBuf->AllocPa);

		if (pDmaBuf->AllocVa == NULL) {
			DebugPrint(ERROR, DBG_INIT,
				   "Failed to allocate a tx ring big buffer\n");
			...
			*pPerRingAllocBuffSz += pDmaBuf->AllocSize;
			DebugPrint(
				TRACE, DBG_INIT,
				"RingIdx %d, Index %d, allocSize %d, PerRingAllocBuffSz %d\n",
				RingIdx, Index, pDmaBuf->AllocSize,
				*pPerRingAllocBuffSz);
		}
...
--*/
NTSTATUS BthInitSend(IN RTBTH_ADAPTER *pAd)
{
	struct rtbt_os_ctrl *os_ctrl = pAd->os_ctrl;
	NTSTATUS Status = STATUS_SUCCESS;
	UCHAR Num; //, Index;
	...
	for (Num = 0; Num < NUM_OF_TX_RING; Num++) {
		DebugPrint(
			TRACE, DBG_INIT,
			"TxRing(Idx:%2d, RingSize:%2d, RingPktSz:%4d, RingOffset:%4d)\n",
			Num, BthGetTxRingSize(pAd, Num),
  ...
  */
	...
	rtbt_dma_mem_alloc(os_ctrl->if_dev, pTxDescBuf->AllocSize, FALSE,
			   &pTxDescBuf->AllocVa, &pTxDescBuf->AllocPa);
	if (pTxDescBuf->AllocVa == NULL) {
		DebugPrint(ERROR, DBG_INIT,
			   "Failed to allocate a Tx desc buffer\n");
		return STATUS_UNSUCCESSFUL;
	}
	//	RingBaseVa = (PVOID)(((UINT32)pTxDescBuf->AllocVa + 0xf) & (~0xf));
	RingBaseVa =
		(PVOID)(((unsigned long)pTxDescBuf->AllocVa + 0xf) & (~0xf));
	...
	for (Num = 0; Num < NUM_OF_TX_RING; Num++) {
		...
		if (BthInitTxRingByIdx(pAd, Num, RingBaseVa, RingBasePaHigh,
				       RingBasePaLow,
				       &PerRingAllocBuffSz) != STATUS_SUCCESS) {
			return Status;
		} else {
			...
			RT_IO_WRITE32(pAd, TX_MAX_CNT0 + Num * RINGREG_DIFF,
				      TxRingSize);
			RT_IO_WRITE32(pAd, TX_CTX_IDX0 + Num * RINGREG_DIFF, 0);

			DebugPrint(
				TRACE, DBG_INIT,
				"RingIdx %d, PerRingAllocBuffSz %d, TotalAllocBuffSz %d\n",
				Num, PerRingAllocBuffSz, TotalAllocBuffSz);
		}
	}
...
NTSTATUS BthInitRecv(IN RTBTH_ADAPTER *pAd)
{
	...
	rtbt_dma_mem_alloc(os_ctrl->if_dev,
			   NUM_OF_RX_RING * RX_RING_SIZE * RXD_SIZE, FALSE,
			   &RingBaseVa, &RingBasePa);
	if (RingBaseVa == NULL) {
		DebugPrint(ERROR, DBG_INIT,
			   "Failed to allocate a Rx descriptor big buffer\n");
    ...
    */
		RingPa = ((Num == 0) ? RingBasePa :
				       (Num * RX_RING_SIZE * RXD_SIZE));
		RT_IO_WRITE32(pAd, RX_BASE_PTR0 + Num * RINGREG_DIFF, RingPa);
		RT_IO_WRITE32(pAd, RX_MAX_CNT + Num * RINGREG_DIFF,
			      RX_RING_SIZE);
		...
		for (Index = 0; Index < RX_RING_SIZE; Index++) {
			...
			rtbt_dma_mem_alloc(os_ctrl->if_dev, pDmaBuf->AllocSize,
					   FALSE, &pDmaBuf->AllocVa,
					   &pDmaBuf->AllocPa);
			if (pDmaBuf->AllocVa == NULL) {
				DebugPrint(
					ERROR, DBG_INIT,
					"RxRing[%d]:Failed to allocate rx buffer for cell[%d]\n",
					Num, Index);
				Status = STATUS_UNSUCCESSFUL;
				return Status;
			}
...
VOID BthFreeRfd(IN RTBTH_ADAPTER *pAd)
{
	...
	for (RingIdx = 0; RingIdx < NUM_OF_RX_RING; RingIdx++) {
		for (index = 0; index < RX_RING_SIZE; index++) {
			...
			pDmaCb->AllocVa =
				NULL; // This is freed above. Assign null directly.
			pDmaBuf = &pAd->RxRing[RingIdx].Cell[index].DmaBuf;
			if (pDmaBuf->AllocVa != NULL) {
				rtbt_dma_mem_free(os_ctrl->if_dev,
						  pDmaBuf->AllocSize,
						  pDmaBuf->AllocVa,
						  pDmaBuf->AllocPa);
	...
	if (pRxDescRing->AllocVa != NULL) {
		rtbt_dma_mem_free(os_ctrl->if_dev, pRxDescRing->AllocSize,
				  pRxDescRing->AllocVa, pRxDescRing->AllocPa);
	...
	for (RingIdx = 0; RingIdx < NUM_OF_TX_RING; RingIdx++) {
		...
		TxRingSize = BthGetTxRingSize(pAd, RingIdx);
		for (index = 0; index < TxRingSize; index++) {
			pAd->TxRing[RingIdx].Cell[index].AllocVa =
				NULL; // This is freed above. Assign null directly.
	...
	if (pDmaBuf->AllocVa != NULL) {
		rtbt_dma_mem_free(os_ctrl->if_dev, pDmaBuf->AllocSize,
				  pDmaBuf->AllocVa, pDmaBuf->AllocPa);
		pDmaBuf->AllocVa = NULL;
	}
...
VOID RtbtResetPDMA(IN RTBTH_ADAPTER *pAd)
{
	...
	for (Num = 0; Num < NUM_OF_TX_RING; Num++) {
		...
		regVal = (Num > 0) ? BthGetTxRingOffset(pAd, Num) :
	...
	for (Num = 0; Num < NUM_OF_RX_RING; Num++) {
		...
		RingPa = ((Num == 0) ? pAd->RxDescRing[Num].AllocPa :
				       (Num * RX_RING_SIZE * RXD_SIZE));
		RT_IO_WRITE32(pAd, RX_BASE_PTR0 + Num * RINGREG_DIFF, RingPa);
		//---Add by shiang for debug
		RT_IO_WRITE32(pAd, RX_MAX_CNT + Num * RINGREG_DIFF,
...
VOID BthRxPacket(PRTBTH_ADAPTER pAd, UCHAR RingIdx, ULONG RxDmaIdx,
		 PULONG pSwReadIndex)
{
	//	BOOLEAN			bConsumed;
	PRXD_STRUC pRxD;
	...
	DebugPrint(
		TRACE, DBG_MISC,
		"BthRxPacket ..... GET = %d, RxDmaIdx = 0x%08x, pSwReadIndex = 0x%08x\n",
		GET, RxDmaIdx, *pSwReadIndex);

	// A for loop to receive all packets that are readable.
	for (Count = 0; Count < GET; Count++) {
		pRxD = (PRXD_STRUC)pAd->RxRing[RingIdx]
			       .Cell[*pSwReadIndex]
			       .AllocVa;
		pData = (PULONG)pAd->RxRing[RingIdx]
				.Cell[*pSwReadIndex]
				.DmaBuf.AllocVa;
		//  Sanity check
		if (BthPacketFilter(pAd, RingIdx, RxDmaIdx, pSwReadIndex) ==
		    FALSE) {
			// DebugPrint(TRACE, DBG_PDMA_DATA, "BthPacketFilter break. RingIdx = %d,
		...
		rtbth_rx_packet(pAd, *(pRxBI),
				((PUINT8)pData) + sizeof(RXBI_STRUC),
				pRxBI->Len);
		...
		//		DebugPrint(INFO, DBG_PDMA_DATA, "SDL0 = %d. RxBI = %x, %x, %x, %x. New
		//SwReadIndex = %d\n", 		  					 PacketLen, *pData, *(pData+1), *(pData+2),
		//*(pData+3), *pSwReadIndex);
	}
}
...
--*/
VOID BthHandleRecvInterrupt(IN PRTBTH_ADAPTER pAd)
{
	ULONG RxSwReadIdx = 0;
	ULONG RxDmaIdx = 0;
	// PRXD_STRUC		pRxD;
	...
	for (RingIdx = 0; RingIdx < NUM_OF_RX_RING; RingIdx++) {
		...
		RT_IO_READ32(pAd, (RX_DRX_IDX + RingIdx * RINGREG_DIFF),
			     &RxDmaIdx);

		//		DebugPrint(INFO, DBG_PDMA_DATA, "BthHandleRecvInterrupt - RingIdx=%d,
		//RxDmaIdx=%d. RxSwReadIdx = %d. RxCpuIdx = %d)\n", 		  					  RingIdx, RxDmaIdx,
		//pAd->RxRing[RingIdx].RxSwReadIdx, pAd->RxRing[RingIdx].RxCpuIdx);
		...
		// If NICRxPacket successfully. update necessary parameter and write to
		// register.
		if (pAd->RxRing[RingIdx].RxSwReadIdx != RxSwReadIdx) {
			pAd->RxRing[RingIdx].RxSwReadIdx = RxSwReadIdx;
			pAd->RxRing[RingIdx].RxCpuIdx =
				(RxSwReadIdx == 0) ? (RX_RING_SIZE - 1) :
						     (RxSwReadIdx - 1);
			RT_IO_WRITE32(pAd,
				      (RX_CRX_IDX + RingIdx * RINGREG_DIFF),
				      pAd->RxRing[RingIdx].RxCpuIdx);
		}
...
BOOLEAN BthPacketFilter(PRTBTH_ADAPTER pAd, UCHAR RingIdx, ULONG RxDmaIdx,
			PULONG pSwReadIndex)
{
	PRXD_STRUC pRxD;

	if (*pSwReadIndex == RxDmaIdx) {
		//		DebugPrint(TRACE, DBG_MISC, "3298 RxDone - No packets in RX Int
		//(pAd->RxRing[%d].RxSwReadIdx=%d)\n", 								RingIdx,
		//pAd->RxRing[RingIdx].RxSwReadIdx);
		return FALSE;
	}
...
--*/
VOID BthHandleTxRingDmaDoneInterrupt(IN PRTBTH_ADAPTER pAd,
				     IN INT_SOURCE_CSR_STRUC TxRingBitmap)
{
	...
	for (RingIdx = 0; RingIdx < NUM_OF_TX_RING; RingIdx++) {
		...
		RT_IO_READ32(pAd, TX_DTX_IDX0 + RingIdx * RINGREG_DIFF,
			     &pTxRing->TxDmaIdx);
		while ((pTxRing->TxDmaIdx != 0xffffffff) &&
...
#endif
			pTxD = (PTXD_STRUC)(pTxRing->Cell[pTxRing->TxSwFreeIdx]
						    .AllocVa);
			if (pTxD->DmaDone == 0) {
				break;
			}
			...
			// DebugPrint(TRACE, DBG_MISC, "-->%s(TxRing-%d):pTxRing->TxDmaIdx=0x%x,
			// pTxRing->TxSwFreeIdx=0x%x!\n", __FUNCTION__, RingIdx,
			// pTxRing->TxDmaIdx, pTxRing->TxSwFreeIdx);
			TxRingSize = BthGetTxRingSize(pAd, RingIdx);
...
int rtbt_pci_isr(IN void *handle)
{
	...
	if (IntSource.word == 0xffffffff) {
		// if (!RT_TEST_FLAG(fdoData, fRTMP_ADAPTER_SLEEP_IN_PROGRESS))
		{
			...
			DebugPrint(
				ERROR, DBG_INIT,
				"Hardware has been removed!(Surprise Removal) \n");
		}
		return -1;
	}
