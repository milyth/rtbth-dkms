...
#define TX_PSBU_RING_BASE_PTR0_OFFSET \
	(TX_ASBU_RING_BASE_PTR0_OFFSET + (TX_ASBU_RING_SIZE * TXD_SIZE))
#define TX_PSBC_RING_BASE_PTR0_OFFSET \
	(TX_PSBU_RING_BASE_PTR0_OFFSET + (TX_PSBU_RING_SIZE * TXD_SIZE))
#define TX_SYNC_RING_BASE_PTR0_OFFSET_BASE \
	(TX_PSBC_RING_BASE_PTR0_OFFSET + (TX_PSBC_RING_SIZE * TXD_SIZE))
#define TX_ACLU_RING_BASE_PTR0_OFFSET_BASE    \
	(TX_SYNC_RING_BASE_PTR0_OFFSET_BASE + \
	 (TX_SYNC_RING_SIZE * TXD_SIZE * NUM_OF_SYNC_TX_RING))
#define TX_ACLC_RING_BASE_PTR0_OFFSET_BASE    \
	(TX_ACLU_RING_BASE_PTR0_OFFSET_BASE + \
	 (TX_ACLU_RING_SIZE * TXD_SIZE * NUM_OF_ACLU_TX_RING))
#define TX_LEACL_RING_BASE_PTR0_OFFSET_BASE   \
	(TX_ACLC_RING_BASE_PTR0_OFFSET_BASE + \
	 (TX_ACLC_RING_SIZE * TXD_SIZE * NUM_OF_ACLC_TX_RING))

#define TX_RING_TOTAL_DESC_NUM                     \
	(TX_ASBU_RING_SIZE * NUM_OF_ASBU_TX_RING + \
...
#define RT_IO_READ32(_A, _R, _pV)                                            \
	do {                                                                 \
		...
		if (((_A)->btFunCtrl.word & 0x01) == 0x01) {                 \
			RT_PCI_IO_READ32((ULONG)(((_A)->CSRAddress) + (_R)), \
					 (UINT32 *)(&_vValue));              \
			*(_pV) = (UINT32)_vValue;                            \
		} else {                                                     \
			_vValue = 0xffffffff;                                \
...
#define RT_IO_WRITE32(_A, _R, _V)                                             \
	do {                                                                  \
		if (((_A)->btFunCtrl.word & 0x01) == 0x01) {                  \
			RT_PCI_IO_WRITE32((ULONG)(((_A)->CSRAddress) + (_R)), \
					  (UINT32)(_V));                      \
		}                                                             \
	} while (0)

#define RT_IO_FORCE_READ32(_A, _R, _pV) \
	RT_PCI_IO_READ32((ULONG)(((_A)->CSRAddress) + (_R)), (UINT32 *)_pV)
#define RT_IO_FORCE_WRITE32(_A, _R, _V) \
	RT_PCI_IO_WRITE32((ULONG)(((_A)->CSRAddress) + (_R)), (UINT32)(_V))

UINT8 BthGetTxRingSize(IN struct _RTBTH_ADAPTER *pAd, //sean wang linux
		       IN UINT8 RingIdx);

UINT16 BthGetTxRingPacketSize(IN struct _RTBTH_ADAPTER *pAd, //sean wang linux
			      IN UINT8 RingIdx);

UINT32 BthGetTxRingOffset(IN struct _RTBTH_ADAPTER *pAd, //sean wang linux
			  IN UINT8 RingIdx);
...
BOOLEAN BthPacketFilter(struct _RTBTH_ADAPTER *pAd, //sean wang linux
			UCHAR RingIdx, ULONG RxDmaIdx, PULONG pSwReadIndex);
...
NTSTATUS
BthReInitSendDesc(struct _RTBTH_ADAPTER *pAd); //sean wang

NTSTATUS
BthReInitRecvDesc(struct _RTBTH_ADAPTER *pAd); //sean wang

VOID RTMPFindPCIePowerLinkCtrl(struct _RTBTH_ADAPTER *pAd); //sean wang

VOID RTMPrt3xSetPCIePowerLinkCtrl(struct _RTBTH_ADAPTER *pAd); //sean wang

ULONG RTMPReadCBConfig(IN ULONG Bus, IN ULONG Slot, IN ULONG Func,
		       IN ULONG Offset);

VOID RTMPWriteCBConfig(IN ULONG Bus, IN ULONG Slot, IN ULONG Func,
		       IN ULONG Offset, IN ULONG Value);
