...
typedef struct _RXBI_STRUC {
	...
	unsigned int
		FhsId : 2; // 0: Inquiry result, 1: page scan result, 2: role switch result
	...
	unsigned int Rsv3 : 3;
	unsigned int
		IntrSltCnt : 11; // the intraslot time of the beginning of the current packet
	unsigned int Rsv4 : 5;
	//Word2
	unsigned int
		BtClk : 28; // the BT clock of the beginning of the current packet
	unsigned int Rsv5 : 4;
	//Word3
	unsigned int
		Rssi : 8; // the received signal strength of the current packet
	unsigned int
		Lna : 2; // the received signal LNA gain of the current packet
