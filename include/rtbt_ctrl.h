...
#define TX_PACKET_SIZE \
	(16 + 1024) // TXBI + 3DH5(1021 bytes) // TODO: ACLC only needs 17 bytes
#define RX_PACKET_SIZE \
	(16 + 1024 +   \
	 4) // RXBI + 3DH5(1021 bytes) + 4(HW) // TODO: ACLC only needs 17 bytes
...
static inline void dump_list(LIST_ENTRY *listHd)
{
	...
	if (listHd && listHd->Flink)
		DebugPrint(TRACE, DBG_MISC,
			   "Dump_list, Head=0x%x, Flink=0x%x, Blink=0x%x\n",
			   listHd, listHd->Flink, listHd->Blink);
	else
		DebugPrint(TRACE, DBG_MISC,
			   "Dump_list error: List not inited\n");

	tmp = listHd->Flink;
	while (tmp != listHd) {
		DebugPrint(TRACE, DBG_MISC,
			   "Entry(0x%x)--Flink:0x%x, Blink:0x%x\n", tmp,
			   tmp->Flink, tmp->Blink);
		tmp = tmp->Flink;
	};
}
...
static char *list_op_str[] = {
	"NoOp",		  "InsertHeadList",  "InsertTailList",
	"RemoveHeadList", "RemoveEntryList",
};

static inline void check_list_constant(LIST_ENTRY *listHd, enum LIST_OP op,
				       LIST_ENTRY *entry, const char *func_str)
{
	LIST_ENTRY *ptr;
	...
	if ((listHd->Flink == listHd || listHd->Blink == listHd) &&
		...
		DebugPrint(
			ERROR, DBG_MISC,
			"List Check Error in function(%s) for OpCode(%s), List still empty!\n",
	...
	if (op == LIST_ADD_HEAD) {
		if ((ptr != entry) || (entry->Blink != listHd)) {
			DebugPrint(
				ERROR, DBG_MISC,
				"List Check Error in function(%s) for OpCode(%s), Not insert to the list head!\n",
				func_str, list_op_str[op]);
			DebugPrint(
				ERROR, DBG_MISC,
				"\tHead=0x%x(F:0x%x, B:0x%x), add_entry=0x%x(F:0x%x, B:0x%x)!\n",
				listHd, listHd->Flink, entry, entry->Flink,
				entry->Blink);
			err = 1;
			goto done;
		}
	...
	while ((ptr != listHd) && (stop_cnt < 200)) {
		if ((op == LIST_DEL_ENTRY) && (ptr == entry)) {
			DebugPrint(
				ERROR, DBG_MISC,
				"List Check Error in function(%s) for OpCode(%s), Not deleted from the list!\n",
				func_str, list_op_str[op]);
			DebugPrint(
				ERROR, DBG_MISC,
				"\tHead=0x%x(F:0x%x, B:0x%x), del_entry=0x%x(F:0x%x, B:0x%x)!\n",
				listHd, listHd->Flink, entry, entry->Flink,
				entry->Blink, ptr);
			err = 1;
			goto done;
	...
	if (stop_cnt >= 200) {
		DebugPrint(
			ERROR, DBG_MISC,
			"List Check Error in function(%s) for OpCode(%s), this list more than 200 entities when do forwarding check!\n",
			func_str, list_op_str[op]);
		goto done;
	}
	...
	if (op == LIST_ADD_TAIL) {
		if ((ptr != entry) || (entry->Flink != listHd)) {
			DebugPrint(
				ERROR, DBG_MISC,
				"List Check Error in function(%s) for OpCode(%s), Not insert to the list tail!\n",
				func_str, list_op_str[op]);
			DebugPrint(
				ERROR, DBG_MISC,
				"\tHead=0x%x(F:0x%x, B:0x%x), add_entry=0x%x(F:0x%x, B:0x%x)!\n",
	...
	if (bcnt != fcnt) {
		DebugPrint(
			ERROR, DBG_MISC,
			"List Check Error in function(%s) for OpCode(%s), foward(%d) and backword(%d) not match!\n",
	...
	if (stop_cnt >= 200) {
		DebugPrint(
			ERROR, DBG_MISC,
			"List Check Error in function(%s) for OpCode(%s), this list more than 200 entities when do backward check!\n",
			func_str, list_op_str[op]);
		goto done;
	}
	...
	if (err) {
		DebugPrint(ERROR, DBG_MISC,
			   "List Check Error, Dump whole link list out!\n");
		dump_list(listHd);
	}
}
...
#define InitializeListHead(_list)                                 \
	do {                                                      \
		...
		check_list_constant(_list, LIST_DO_NOTHING, NULL, \
				    __FUNCTION__);                \
	} while (0)
...
#define InsertHeadList(_listHd, _entry)                             \
	do {                                                        \
		...
		_tmp->Blink = (_entry);                             \
		check_list_constant(_listHd, LIST_ADD_HEAD, _entry, \
				    __FUNCTION__);                  \
	} while (0)
...
#define InsertTailList(_listHd, _entry)                             \
	do {                                                        \
		...
		check_list_constant(_listHd, LIST_ADD_TAIL, _entry, \
				    __FUNCTION__);                  \
	} while (0)
...
#define RemoveHeadList(_listHd)                                      \
	...
	do {                                                         \
		...
		check_list_constant(_listHd, LIST_DEL_HEAD, NULL,    \
				    __FUNCTION__);                   \
	} while (0)
...
#define RemoveEntryList(_entry)                                               \
	do {                                                                  \
		if ((_entry) && ((_entry)->Flink != (_entry)) &&              \
			...
			check_list_constant(_f_entry, LIST_DEL_ENTRY, _entry, \
					    __FUNCTION__);                    \
		}                                                             \
	} while (0)
...
BOOLEAN RtbtReceivePacket(IN PRTBTH_ADAPTER pAd, IN UINT32 SwReadIndex,
			  IN PRXBI_STRUC pRxBI, IN PUINT8 pRxPayload,
			  IN ULONG RxPayloadLen);

VOID FillTxPacketForTest(IN PRTBTH_ADAPTER pAd, IN UCHAR RingIdx);
